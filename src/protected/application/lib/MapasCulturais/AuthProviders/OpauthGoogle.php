...
class OpauthGoogle extends \MapasCulturais\AuthProvider{
    protected $opauth;
    ...
    protected function _init() {
        ...
        $app->hook('GET(auth.index)', function () use($app){
            $app->redirect($this->createUrl('google'));
        });

        $app->hook('<<GET|POST>>(auth.Google)', function () use($opauth, $config){
//            $_POST['openid_url'] = $config['login_url'];
    ...
    public function _getAuthenticatedUser() {
        ...
        if($this->_validateResponse()){
            ...
            $auth_provider = $app->getRegisteredAuthProviderId('Google');
